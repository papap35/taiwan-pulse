...
| 疫情監測（與空氣品質共用色相，方形標記） | 疾病管制署 法定傳染病監測，登革熱/流感/腸病毒/COVID 超過門檻才顯示 | 不需金鑰，但預設沒有端點，需自行設定 `CDC_EPIDEMIC_URL` | https://data.cdc.gov.tw/ |
...
**未經真實驗證**，見 SPEC.md P0-1。`lib/sources/epidemic.ts`
（`CDC_EPIDEMIC_URL`）連預設端點都沒有給，因為連猜測用的網址都沒有把握，
需要你自行到 https://data.cdc.gov.tw/ 找到正確的資料集網址。
